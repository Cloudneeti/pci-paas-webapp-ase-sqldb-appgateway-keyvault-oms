...
##  Author - Shabuddin Khan | Azure Cloud Architect
##  Organisation - Avyan Consulting Corp
##  Email - Shabuddink@cloudneeti.com / Shabuddink@avyanconsulting.com
##  Version - 1.0.0
################################################################################
...
Begin
{
	...
	$automationaccname = "automationacc" + (Get-Random -Maximum 999)
	$AutomationADApplication = "AutomationAppl" + (Get-Random -Maximum 9999)
	$TemplateUri = "https://raw.githubusercontent.com/AvyanConsultingCorp/pci-paas-webapp-ase-sqldb-appgateway-keyvault-oms/master/azuredeploy.json"
...
Process
{
    Write-Output "`nDownloading Scripts from GitHub..."
    try {
    ...
    .\Scripts\Install-azure-powershell-modules.ps1
	...
	Write-Output "`nCreating RunAs account for runbooks to execute."
    ...
    if ($DeploymentStatus -eq 'Succeeded')
      ...
      {
            Write-Error "Deployment deploy-SQLServerSQLDb has failed. Hence, Post-Deployment script has failed to execute."
      }
#>  
}
